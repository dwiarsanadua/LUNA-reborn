...
int main() {
    ...
    // ─── Test 3: PartySystem (single member) ───
    {
        PartySystem party;
    ...
    // ─── QA-03 Test 1: Party EXP Share (4 players) ───
    {
        PartySystem party;
        auto leader = reg.create(), m1 = reg.create(), m2 = reg.create(), m3 = reg.create();
        for (auto e : {leader, m1, m2, m3}) reg.emplace<CharacterStats>(e);
        TEST("Create 4-player party", party.CreateParty(reg, leader));
        TEST("Invite m1", party.InviteToParty(reg, leader, m1));
        TEST("m1 accepts", party.AcceptInvite(reg, m1));
        TEST("Invite m2", party.InviteToParty(reg, leader, m2));
        TEST("m2 accepts", party.AcceptInvite(reg, m2));
        TEST("Invite m3", party.InviteToParty(reg, leader, m3));
        TEST("m3 accepts", party.AcceptInvite(reg, m3));
        auto& p = reg.get<Party>(leader);
        TEST("Party has 4 members", p.members.size() == 4);
        uint32_t shared = party.DistributeXP(reg, leader, 1000);
        TEST("EXP distributed > 0", shared > 0);
    }

    // ─── QA-03 Test 2: Guild + Chat ───
    {
        GuildSystem guild;
        auto founder = reg.create(), member = reg.create();
        reg.emplace<CharacterStats>(founder); reg.emplace<CharacterStats>(member);
        TEST("Create guild for chat test", guild.CreateGuild(reg, founder, "GuildChatTest"));
        TEST("Invite to guild chat", guild.InviteToGuild(reg, founder, member));
        TEST("Accept guild chat invite", guild.AcceptInvite(reg, member));
        guild.GuildChat(reg, founder, "Hello!");
        TEST("Guild chat no crash", true);
    }
    ...
    // ─── QA-03 Test 3: PVP Combat ───
    {
        ...
        if (normal.damage > 0) {
            ...
            TEST("PvP damage ~35% of Normal", ratio > 0.2f && ratio < 0.5f);
        } else {
            TEST("PvP damage (normal dealt 0)", pvp.damage == 0);
        }
    }

    // ─── QA-03 Test 4: Cleanup on Disconnect ───
    // (covered by existing LeaveParty test above; additional edge case)
    {
        PartySystem party;
        auto leader = reg.create(), leaver = reg.create();
        reg.emplace<CharacterStats>(leader); reg.emplace<CharacterStats>(leaver);
        party.CreateParty(reg, leader);
        party.InviteToParty(reg, leader, leaver); party.AcceptInvite(reg, leaver);
        party.LeaveParty(reg, leaver);
        TEST("Leaver has no Party component", !reg.all_of<Party>(leaver));
    }
