...
void BuffSystem::Update(float dt, CharacterStats& stats) {
    for (auto& buff : active_buffs_) {
        if (buff.expired || buff.permanent) continue;
        buff.elapsed += dt;
        ...
        if (buff.tick_interval > 0) {
            ...
            while (buff.tick_timer >= buff.tick_interval) {
                ...
                if (buff.type == BuffType::DamageOverTime) stats.hp -= buff.tick_damage;
                if (buff.type == BuffType::HealOverTime) stats.hp = std::min(stats.max_hp, stats.hp + buff.tick_heal);
            }
        }
...
int BuffSystem::Apply(const BuffInstance& buff, CharacterStats& stats) {
    if (buff.stack_rule == BuffStackRule::Exclusive) {
        for (auto& b : active_buffs_) {
            if (b.name == buff.name) { b.elapsed = 0; return b.id; }
        }
    }
    if (buff.stack_rule == BuffStackRule::None) {
        for (auto& b : active_buffs_) { if (b.name == buff.name) { b.elapsed = 0; return b.id; } }
    }
    if (buff.stack_rule == BuffStackRule::Intensity) {
        for (auto& b : active_buffs_) {
            if (b.name == buff.name && b.current_stacks < b.max_stacks) {
                b.current_stacks++; b.elapsed = 0; return b.id;
            }
        }
    }
...
void BuffSystem::Remove(int buff_id) {
    for (auto& b : active_buffs_) { if (b.id == buff_id) { b.expired = true; break; } }
}

void BuffSystem::RemoveByType(BuffType type) {
    for (auto& b : active_buffs_) { if (b.type == type) b.expired = true; }
}

void BuffSystem::RemoveByName(const std::string& name) {
    for (auto& b : active_buffs_) { if (b.name == name) b.expired = true; }
}
