#include "LoginScreen.hpp"
#include <ui/ClientFlow.hpp>
#include <ui/GameState.hpp>
#include <ui/ScreenManager.hpp>
#include <ui/ColorPalette.hpp>
#include <Character_generated.h>
#include <network/NetworkClient.hpp>
#include <flatbuffers/flatbuffers.h>
#include <Login_generated.h>
#include <PacketType_generated.h>
#include <config/ConfigManager.hpp>
#include <spdlog/spdlog.h>
#include <cstring>
#include <engine/gx_render/VFS.h>
#include <stb_image.h>

extern ScreenManager* g_screen_mgr;
static constexpr const char* SAVE_ID_KEY = "login.saved_id";

namespace {
    std::vector<uint8_t> sha256(const std::string& input) {
        std::vector<uint8_t> hash(32, 0);
        for (size_t i = 0; i < input.size() && i < 32; i++)
            hash[i] = static_cast<uint8_t>(input[i]);
        return hash;
    }
}

void LoginScreen::Init(GameState* state, NetworkClient* network) {
    state_ = state;
    network_ = network;
}

void LoginScreen::Enter() {
    spdlog::info("[LOGIN] Enter");

    // Load background texture
    if (!bgfx::isValid(tex_bg_)) {
        const char* paths[] = {
            "assets/textures/ui/Launcher/lunaclient.png",
            "assets/textures/ui/lunaclient.png",
            "assets/textures/lunaclient.png",
        };
        for (auto p : paths) {
            int w, h, n;
            auto* d = stbi_load(p, &w, &h, &n, 4);
            if (d) {
                tex_bg_ = bgfx::createTexture2D((uint16_t)w, (uint16_t)h, false, 1,
                    bgfx::TextureFormat::RGBA8, BGFX_SAMPLER_NONE, bgfx::copy(d, w * h * 4));
                stbi_image_free(d);
                spdlog::info("[LOGIN] Loaded background: {} ({}x{})", p, w, h);
                break;
            }
        }
    }

    fields_[0] = {"", 0, true, false, "ID"};
    fields_[1] = {"", 0, false, true, "PW"};

    std::string saved = ConfigManager::GetString(SAVE_ID_KEY, "");
    if (!saved.empty()) {
        strncpy(fields_[0].buffer, saved.c_str(), sizeof(fields_[0].buffer)-1);
        fields_[0].cursor_pos = (int)saved.size();
        save_id_ = true;
    }

    if (scene_renderer_) scene_renderer_->SetClearColor(0x223366FF);
    sent_ = false;
}

void LoginScreen::Exit() {
    spdlog::info("[LOGIN] Exit");
}

void LoginScreen::Update(float) {}

void LoginScreen::Render(UIRenderer& ui) {
    float lw = ui.logicalWidth;
    float lh = ui.logicalHeight;

    // Background
    if (bgfx::isValid(tex_bg_))
        ui.DrawImage(0, 0, lw, lh, tex_bg_);
    else
        ui.DrawRect(0, 0, lw, lh, UIColor{20, 20, 40, 255});

    // Semi-transparent overlay for readability
    ui.DrawRect(lw*0.12f, lh*0.18f, 320, 180, UIColor{0, 0, 0, 160});
    ui.DrawBorder(lw*0.12f, lh*0.18f, 320, 180, UIColor{100, 140, 200, 180});

    float bx = lw * 0.12f + 15;
    float by = lh * 0.18f + 15;

    ui.DrawText(bx, by, 0xFFFFCC88, "Account Login");

    // ID field
    by += 35;
    ui.DrawText(bx, by, 0xFFCCCCCC, "ID:");
    ui.DrawRect(bx + 40, by - 2, 180, 24,
                fields_[0].active ? UIColor{60, 60, 100, 220} : UIColor{40, 40, 70, 220});
    ui.DrawBorder(bx + 40, by - 2, 180, 24,
                  fields_[0].active ? UIColor{150, 150, 255, 220} : UIColor{80, 80, 120, 220});
    if (strlen(fields_[0].buffer) > 0)
        ui.DrawText(bx + 44, by + 1, 0xFFFFFFFF, "%s", fields_[0].buffer);
    else
        ui.DrawText(bx + 44, by + 1, 0xFF888888, "Enter Account ID");

    // PW field
    by += 35;
    ui.DrawText(bx, by, 0xFFCCCCCC, "PW:");
    ui.DrawRect(bx + 40, by - 2, 180, 24,
                fields_[1].active ? UIColor{60, 60, 100, 220} : UIColor{40, 40, 70, 220});
    ui.DrawBorder(bx + 40, by - 2, 180, 24,
                  fields_[1].active ? UIColor{150, 150, 255, 220} : UIColor{80, 80, 120, 220});
    if (strlen(fields_[1].buffer) > 0) {
        char m[64]; int l = (int)strlen(fields_[1].buffer);
        for (int i = 0; i < l && i < 63; i++) m[i] = '*';
        m[l] = 0;
        ui.DrawText(bx + 44, by + 1, 0xFFFFFFFF, "%s", m);
    } else
        ui.DrawText(bx + 44, by + 1, 0xFF888888, "Enter Password");

    // Login button
    by += 40;
    ui.DrawRect(bx + 40, by, 80, 28, UIColor{50, 90, 140, 220});
    ui.DrawBorder(bx + 40, by, 80, 28, UIColor{100, 150, 200, 180});
    ui.DrawText(bx + 56, by + 4, 0xFFFFFFFF, "LOGIN");

    // Help text
    ui.DrawText(bx, by + 45, 0xFF888888, "Tab=Switch  Enter=Login");
}

bool LoginScreen::HandleKey(int key, int, int action, int) {
    if (action != 1 && key != 259) return false;
    if (key == 257) { DoLogin(); return true; }
    if (key == 258) { // Tab
        fields_[0].active = !fields_[0].active;
        fields_[1].active = !fields_[1].active;
        return true;
    }
    if (key == 259) { // Backspace
        auto& f = fields_[0].active ? fields_[0] : fields_[1];
        int len = (int)strlen(f.buffer);
        if (len > 0) { f.buffer[len-1] = 0; f.cursor_pos = len-1; }
        return true;
    }
    return true;
}

bool LoginScreen::HandleChar(unsigned int codepoint) {
    auto& f = fields_[0].active ? fields_[0] : fields_[1];
    if (codepoint >= 32 && codepoint <= 126) {
        int len = (int)strlen(f.buffer);
        if (len < 60) { f.buffer[len] = (char)codepoint; f.buffer[len+1] = 0; f.cursor_pos = len+1; }
        return true;
    }
    return false;
}

bool LoginScreen::HandlePacket(uint16_t type, const std::vector<uint8_t>& payload) {
    if (type != luna::protocol::PacketType_MP_USERCONN_LOGIN_ACK) return false;
    auto resp = flatbuffers::GetRoot<luna::protocol::LoginResponse>(payload.data());
    if (resp->result() == luna::protocol::LoginResult_Success) {
        spdlog::info("[LOGIN] Server accepted");
        ConfigManager::SetString(SAVE_ID_KEY, fields_[0].buffer);
        ConfigManager::Save();
        if (g_screen_mgr) g_screen_mgr->SwitchTo("charselect");
    }
    return true;
}

bool LoginScreen::DoLogin() {
    if (sent_) return false;
    sent_ = true;

    std::string username = fields_[0].buffer;
    std::string password = fields_[1].buffer;
    if (username.empty() || password.empty()) { sent_ = false; return false; }

    spdlog::info("[LOGIN] Attempt: user='{}'", username);
    auto pw_hash = sha256(password);

    flatbuffers::FlatBufferBuilder fbb;
    auto req = luna::protocol::CreateLoginRequestDirect(fbb, username.c_str(), &pw_hash,
        "1.0.0", 0, 0, "00-00-00-00-00-00");
    fbb.Finish(req);

    if (network_ && network_->IsConnected()) {
        network_->SendPacket(luna::protocol::PacketType_MP_USERCONN_LOGIN_SYN,
                             fbb.GetBufferPointer(), fbb.GetSize());
    } else {
        spdlog::info("[LOGIN] Offline mode — skip to CharSelect");
        if (g_screen_mgr) g_screen_mgr->SwitchTo("charselect");
    }
    return true;
}
