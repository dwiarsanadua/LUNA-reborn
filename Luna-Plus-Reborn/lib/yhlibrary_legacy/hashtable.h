...
private:
	...
	protected:
		uint32_t				m_dwDataNum;
		uint32_t				m_dwMaxBucketNum;
		...
		uint32_t				m_MultiDataKey;
		
	public:
		uint32_t	inline		GetDataNum() {return m_dwDataNum;}
		bool				Initialize(uint32_t dwMaxBucketNum)
		{
		...
		bool				Add(T* pVoid,uint32_t dwKey)
		{
			uint32_t index = dwKey % m_dwMaxBucketNum;
		...
		void				StartGetMultiData(uint32_t dwKey)
		{
			uint32_t index = dwKey%m_dwMaxBucketNum;			
		...
		void				RemoveCurMultiData()
		{
			...
			{
				uint32_t dwIndex = m_MultiDataKey%m_dwMaxBucketNum;
		...
		void				Remove(uint32_t dwKey)
		{
			uint32_t dwIndex = dwKey%m_dwMaxBucketNum;
