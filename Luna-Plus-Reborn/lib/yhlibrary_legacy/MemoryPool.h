...
private:
	...
	DATATYPE*		m_pMemory;
	uint32_t		m_dwFreeMemoryIndex;
	uint32_t		m_dwMaxObjectNum;
...
//		LeaveCriticalSection(&m_csMemory);
	...
	void					Init(uint32_t dwNum)
	{
		...
		// ½ÇÁ¦ ¸Þ¸ð¸® »ý¼º 
		for( uint32_t i =0;i<m_dwMaxObjectNum;i++)
		{
...
//		LeaveCriticalSection(&m_csMemory);
	...
	uint32_t					GetFreeIndexNum()
	{
//		EnterCriticalSection(&m_csMemory);
		uint32_t temp  = m_dwFreeMemoryIndex;
//		LeaveCriticalSection(&m_csMemory);
		return temp;
	}
	CMemoryPool()
