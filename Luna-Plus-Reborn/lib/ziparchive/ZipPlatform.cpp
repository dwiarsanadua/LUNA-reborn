...
const char CZipPathComponent::m_cSeparator = _T('\\');
...
uint32_t ZipPlatform::GetDeviceFreeSpace(const char* lpszPath)
{
	uint32_t SectorsPerCluster, BytesPerSector, NumberOfFreeClusters, TotalNumberOfClusters;
...
bool ZipPlatform::GetFileSize(const char* lpszFileName, uint32_t& dSize)
{
	void* f = CreateFile(lpszFileName, GENERIC_READ, FILE_SHARE_READ, 
...
CZipString ZipPlatform::GetTmpFileName(const char* lpszPath, uint32_t iSizeNeeded)
{
		char empty[] = _T("");
		CZipString tempPath;
		...
		if (bCheckTemp)
		{
			uint32_t size = GetTempPath(0, NULL);
			if (size == 0)
				return empty;
...
{
	uint32_t i = ::GetCurrentDirectory(0, NULL);
	if (!i)
		return false;
...
bool ZipPlatform::SetFileAttr(const char* lpFileName, uint32_t uAttr)
{
	return ::SetFileAttributes(lpFileName, uAttr) != 0;
}

bool ZipPlatform::GetFileAttr(const char* lpFileName, uint32_t& uAttr)
{
	// not using MFC due to MFC bug (attr is one byte there)
	uint32_t temp = ::GetFileAttributes(lpFileName);
	if (temp == -1)
		return false;
	uAttr = temp;
	return true;
...
bool ZipPlatform::GetFileModTime(const char* lpFileName, time_t & ttime)
{
#if defined _MSC_VER && !defined __BORLANDC__ /*_MSC_VER may be defined in Borland after converting the VC project */
    struct _stat st;
...
bool ZipPlatform::SetFileModTime(const char* lpFileName, time_t ttime)
{
	struct _utimbuf ub;
	ub.actime = time(NULL);
...
bool ZipPlatform::ChangeDirectory(const char* lpDirectory)
{
	return _tchdir(lpDirectory) == 0; // returns 0 if ok
}
int ZipPlatform::FileExists(const char* lpszName)
{
...
ZIPINLINE  bool ZipPlatform::IsDriveRemovable(const char* lpszFilePath)
{
	CZipPathComponent zpc(lpszFilePath);
	return ::GetDriveType(zpc.GetFileDrive()) == DRIVE_REMOVABLE;
}

ZIPINLINE  bool ZipPlatform::SetVolLabel(const char* lpszPath, const char* lpszLabel)
{
	CZipPathComponent zpc(lpszPath);
	CZipString szDrive = zpc.GetFileDrive();
...
ZIPINLINE  bool ZipPlatform::RemoveFile(const char* lpszFileName, bool bThrow)
{
...
ZIPINLINE  bool ZipPlatform::RenameFile( const char* lpszOldName, const char* lpszNewName, bool bThrow)
{
...
ZIPINLINE  bool ZipPlatform::IsDirectory(uint32_t uAttr)
{
	return (uAttr & FILE_ATTRIBUTE_DIRECTORY) != 0;
}
ZIPINLINE  bool ZipPlatform::CreateDirectory(const char* lpDirectory)
{
	return ::CreateDirectory(lpDirectory, NULL) != 0;
}

ZIPINLINE  uint32_t ZipPlatform::GetDefaultAttributes()
{
	return 0x81a40020; // make it readable under Unix
}

ZIPINLINE  uint32_t ZipPlatform::GetDefaultDirAttributes()
{
...
#ifdef _UNICODE	
int ZipPlatform::WideToSingle(const char* lpWide, CZipAutoBuffer &szSingle)
{
...
#include <io.h>
#include <share.h>
bool ZipPlatform::TruncateFile(int iDes, uint32_t iSize)
{
...
int ZipPlatform::OpenFile(const char* lpszFileName, uint32_t iMode, int iShareMode)
{
	switch (iShareMode)
	{
