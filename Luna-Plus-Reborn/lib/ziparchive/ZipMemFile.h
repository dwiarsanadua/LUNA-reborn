...
public:
	...
	void Write(const void* lpBuf, uint32_t nCount);
	uint32_t Read(void* lpBuf, uint32_t nCount);
	...
	CZipMemFile(uint8_t* lpBuf, uint32_t nBufSize, long nGrowBy = 0)
	{
		Init();
		Attach(lpBuf, nBufSize, nGrowBy);
	}
	ZIP_ULONGLONG GetPosition() const {	return m_nPos;}
	void Attach(uint8_t* lpBuf, uint32_t nBufSize, long nGrowBy = 0)
	{
		Close();
		m_lpBuf = lpBuf;
		m_nGrowBy = nGrowBy;
