...
{
	CZipAddNewFileInfo(const char* lpszFilePath, bool bFullPath = true)
		: m_szFilePath(lpszFilePath),m_bFullPath(bFullPath)
	{
		m_pFile = NULL;
		Defaults();
	}
	CZipAddNewFileInfo(const char* lpszFilePath, const char* lpszFileNameInZip)
		: m_szFilePath(lpszFilePath), m_szFileNameInZip(lpszFileNameInZip)
	{
		m_pFile = NULL;
		Defaults();
	}
	CZipAddNewFileInfo(CZipAbstractFile* pFile, const char* lpszFileNameInZip)
	...
	*/
	...
	/**
		...
		uint32_t (-1)	(you need to remember the last argument if you need the size of the file)		
	*/
	int m_iSmartLevel;
...
public:
	...
	*/
		...
		uint32_t m_uUncomprLeft;	///< bytes left to uncompress
		uint32_t m_uComprLeft;		///< bytes left to decompress
		uint32_t m_uCrc32;			///< crc32 file control value
...
*/
	static int WideToSingle(const char* lpWide, CZipAutoBuffer &szSingle);
...
*/
	bool SetPassword(const char* lpszPassword = NULL);
...
*/
	...
	*/
	...
	/**
		Archive open modes used in <CODE> #Open(const char* , int , int ) </CODE>
...
/**
	...
	\see GetSpanMode
*/
	void Open(const char* szPathName, int iMode = zipOpen, int iVolumeSize = 0);
...
*/
	void SetRootPath(const char* szPath = NULL);
	...
	*/
	...
	/**
		...
		\see SetCallback
		\see <CODE> AddNewFile(const char*, const char*, int, int, unsigned long) </CODE>
		\see <CODE> AddNewFile(const char*, int, bool, int, unsigned long) </CODE>
		\see <CODE> AddNewFile(CZipMemFile&, const char*, int, int, unsigned long)</CODE>
	*/
	...
	*/
	bool AddNewFile(const char* lpszFilePath, 
							int iComprLevel = -1,
							bool bFullPath = true,
...
*/
	bool OpenNewFile(CZipFileHeader & header, int iLevel = Z_DEFAULT_COMPRESSION, const char* lpszFilePath = NULL, uint32_t uInternal = 0);
...
/**
	...
	\see OpenNewFile
*/
	bool WriteNewFile(const void *pBuf, uint32_t iSize);
...
/**
	...
	\see OpenNewFile
*/
	void SetExtraField(const char *pBuf, uint16_t iSize);
...
/**
	...
	\see SetAdvanced 
*/
	bool GetFromArchive(CZipArchive& zip, uint16_t uIndex, int iReplaceIndex = -1, bool bKeepSystComp = false)
	{
	...
	\see GetFromArchive(CZipArchive& , uint16_t, int, bool)
...
/**
	...
	- Throws exceptions.
	\see SetCallback
	\see <CODE> ExtractFile(uint16_t, CZipMemFile&, uint32_t) </CODE> 
	\see FindMatches
*/
	bool ExtractFile(uint16_t uIndex,
						const char* lpszPath,
						bool bFullPath = true,
						const char* lpszNewName = NULL,
						uint32_t nBufSize = 65536);

	
	/**
		The same as <CODE> #ExtractFile(uint16_t , const char* , bool , const char* , uint32_t ) </CODE>
	...
	*/
	bool ExtractFile(uint16_t uIndex, CZipMemFile& mf, uint32_t nBufSize = 65536);
...
/**
	...
	\note Throws exceptions.
*/
	uint32_t ReadFile(void *pBuf, uint32_t iSize);
...
*/
	bool TestFile(uint16_t uIndex, uint32_t uBufSize = 65536);
...
/**
	The same as CZipArchive::CloseFile(const char*), but additionally
	closes \e file.
	\param	file
		OPENED CZipFile structure of the extracted file
	\return	
	\note Throws exceptions.
	\see CZipArchive::CloseFile(const char*)
*/
...
/**
	...
	\see FindMatches
*/
	void DeleteFile(uint16_t uIndex);
...
/**
	...
	\note Throws exceptions.
*/
	bool SetGlobalComment(const char* lpszComment);
...
/**
	...
	\note Throws exceptions.
*/
	bool SetFileComment(uint16_t uIndex, const char* lpszComment);
...
*/
	int FindFile(const char* lpszFileName, int iCaseSensitive = ffDefault, bool bFileNameOnly = false);
...
/**
	...
	\return	\c true if successful
*/
	bool GetFileInfo(CZipFileHeader & fhInfo, uint16_t uIndex) const;
...
*/
	void SetFileHeaderAttr(CZipFileHeader& header, uint32_t uAttr);
...
*/
	...
	/**
		...
		\see GetTempPath
	*/
	void SetTempPath(const char* lpszPath = NULL, bool bForce = true);
	...
	*/
     ...
     */
	CZipString PredictFileNameInZip(const char* lpszFilePath, bool bFullPath, int iWhat = prAuto, bool bExactly = false)const ;
	...
	*/
	int WillBeDuplicated(const char* lpszFilePath, bool bFullPath, bool bFileNameOnly = false, int iWhat = prAuto);


    /**
       Predict the full filename with path after extraction. The parameters (except for the first) are in the form you'd pass
	   to the <CODE> #ExtractFile(uint16_t , const char* , bool , const char* , uint32_t ) </CODE> function. The function takes into account the root path set with #SetRootPath.
     ...
     */
	CZipString PredictExtractedFileName(const char* lpszFileNameInZip, const char* lpszPath, bool bFullPath, const char* lpszNewName = NULL)const ;
...
*/
     ...
     */
	static bool RemovePathBeginning(const char* lpszBeginning, CZipString& szPath, ZIPSTRINGCOMPARE pCompareFunction);
	...
	*/
	uint32_t GetCentralDirSize(bool bWhole = true) const
	{
	...
	*/
	...
	public:
		...
		A match means the entire string \e lpszText is used up in matching.
		  ...
		  \param iRetCode
			...
			\see SetPattern
		*/
		bool IsMatch(const char* lpszText, int* iRetCode = NULL);
		 ...
		 */
		static bool IsPattern(const char* lpszPattern);
		 ...
		 */
		static bool IsPatternValid(const char* lpszPattern, int* iErrorType = NULL);
		...
		A match means the entire string \e lpszText is used up in matching.
			...
			one of #Match values
		*/
		static int Match(const char* lpszPattern, const char* lpszText);
		...
		\see SetPattern
		*/
		CWildcard(const char* lpszPattern, bool bCaseSensitive)
		{
		...
		*/
		void SetPattern(const char* lpszPattern, bool bCaseSensitive)
		{
		...
		operator const char*()
		{
			return (const char*)m_szPattern;
		}
	protected:
		bool m_bCaseSensitive;		
		static int MatchAfterStar(const char* p , const char* t);
     ...
     */
	void FindMatches(const char* lpszPattern, CZipWordArray& ar, bool bFullPath = true)const;
    ...
    /**
       Change the name of the file with the given index.
		...
		\see SetAdvanced 
     */
	bool RenameFile(uint16_t uIndex, const char* lpszNewName);
...
protected:
	...
	*/
	void MakeSpaceForReplace(int iReplaceIndex, uint32_t uTotal, const char* lpszFileName);
	/**
	...
	*/
	...
	/**
		...
		<CODE>typedef int (CZipString::*ZIPSTRINGCOMPARE)( const char* ) const;</CODE>
	*/
...
/**
	...
	Called by #Open(const char*, int, int) and #Open(CZipMemFile&, int).
	\param	iMode
		an opening mode		
	\note Throws exceptions.
*/
...
*/
	void MovePackedFiles(uint32_t uStartOffset, uint32_t uEndOffset, uint32_t uMoveBy, CZipActionCallback* pCallback, bool bForward = false);
	...
	*/
	bool GetFromArchive(CZipArchive& zip, uint16_t uIndex, int iReplaceIndex, bool bKeepSystComp, CZipActionCallback* pCallback);

	/**
		\return whether the code should continue or not
	*/
	bool UpdateReplaceIndex(int& iReplaceIndex, const char* lpszNewFileName);
...
*/
	...
	static void* _zliballoc(void* opaque, uint32_t items, uint32_t size); ///< memory allocator called by the zlib library
...
*/
	void CryptDecodeBuffer(uint32_t uCount);
/**
...
/**
	...
	\return	
*/
	uint32_t CryptCRC32(uint32_t l, char c);
/**
