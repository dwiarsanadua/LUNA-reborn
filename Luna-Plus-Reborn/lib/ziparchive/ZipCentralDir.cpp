...
uint32_t CZipCentralDir::Locate()
{

	// maximum size of end of central dir record
	long uMaxRecordSize = 0xffff + CENTRALDIRSIZE;
	uint32_t uFileSize = m_pStorage->m_pFile->GetLength();

	if ((uint32_t)uMaxRecordSize > uFileSize)
...
void CZipCentralDir::OpenFile(uint16_t uIndex)
{
	uint16_t uLocalExtraFieldSize;
...
// 		// we are at the end already
	...
	m_info.m_uEntriesNumber = (uint16_t)m_headers.GetSize();
	m_info.m_uSize = 0;
	...
	{
		uint32_t uVolumeFree = m_pStorage->VolumeLeft();
		...
		uint32_t uToGrow = GetSize(true) - (4 + m_info.m_uEntriesNumber * (4 + 12)); 
	...
	try
	{
		...
		m_info.m_uThisDisk = (uint16_t)m_pStorage->GetCurrentDisk();
		uint32_t uSize = WriteCentralEnd();
...
void CZipCentralDir::WriteHeaders(CZipActionCallback* pCallback, bool bOneDisk)
{
	...
	m_info.m_uDiskWithCD = (uint16_t)m_pStorage->GetCurrentDisk();
	m_info.m_uOffset = m_pStorage->GetPosition() - m_info.m_uBytesBeforeZip;
	if (!m_info.m_uEntriesNumber)
		return;

	uint16_t iDisk = m_info.m_uDiskWithCD;
	...
	for (int i = 0; i < m_info.m_uEntriesNumber; i++)
	{
		...
		if (m_pStorage->GetCurrentDisk() != iDisk)
		{
			...
			iDisk = (uint16_t)m_pStorage->GetCurrentDisk();
...
uint32_t CZipCentralDir::WriteCentralEnd()
{
	uint32_t uSize = GetSize();
	CZipAutoBuffer buf(uSize);
	uint16_t uCommentSize = (uint16_t)m_pszComment.GetSize();
...
uint32_t CZipCentralDir::GetSize(bool bWhole) const
{
	uint32_t uHeaders = 0;
	int iCount = m_headers.GetSize();
	if (bWhole)
...
bool CZipCentralDir::RemoveDataDescr(bool bFromBuffer)
{
	...
	uint32_t uOffsetToChange = 4;
	uint32_t uPosInBuffer = 0;
	uint32_t uExtraHeaderLen;
	int iCount = m_headers.GetSize();
	for (int i = 0; i < iCount; i++)
	{
...
// 		int uDataDescr = (m_headers[i]->m_uFlag & 8) ? (4 + 12) : 0;
		...
		uint32_t uToCopy = (i == (iCount - 1) ? uSize : m_headers[i + 1]->m_uOffset)
...
int CZipCentralDir::FindFile(const char* lpszFileName, bool bCaseSensitive, bool bSporadically, bool bFileNameOnly)
{
	...
	else
	{
		if (bSporadically)
		{
			...
			int iSize = m_findarray.GetSize();
			for (int i = 0; i < iSize; i++)
				if (CompareElement(lpszFileName, (uint16_t)i) == 0)
				{
					iResult = i;
					break;
				}
...
void CZipCentralDir::InsertFindFastElement(CZipFileHeader* pHeader, uint16_t uIndex)
{
	...
	while ( start < end )
	{
		...
		int result = CompareElement(fileName, (uint16_t)midpoint);
	...
	m_findarray.InsertAt(start, CZipFindFast(pHeader, uint16_t(uIndex == uint16_t(-1) ? iSize : uIndex /* just in case */))); 
}

int CZipCentralDir::FindFileNameIndex(const char* lpszFileName) const
{
	int start = 0;
	...
	while ( start <= end )
	{
		...
		int result = CompareElement(lpszFileName, (uint16_t)midpoint);
...
void CZipCentralDir::RenameFile(uint16_t uIndex, const char* lpszNewName)
{
