...
typedef uint32_t(*conv_func)(uint32_t , bool );

uint32_t AttrDos(uint32_t , bool );
uint32_t AttrUnix(uint32_t, bool);
uint32_t AttrMac(uint32_t , bool );
...
uint32_t ZipCompatibility::ConvertToSystem(uint32_t uAttr, int iFromSystem, int iToSystem)
{
...
uint32_t AttrDos(uint32_t uAttr, bool )
{
	return uAttr;	
}
...
uint32_t AttrUnix(uint32_t uAttr, bool bFrom)
{
	uint32_t uNewAttr = 0;
	if (bFrom)
	{
...
uint32_t AttrMac(uint32_t uAttr, bool )
{
	uint32_t uNewAttr  = uAttr & (attDir | attROnly);
// 	if (bFrom)
// 	{
// 		
// 	}
