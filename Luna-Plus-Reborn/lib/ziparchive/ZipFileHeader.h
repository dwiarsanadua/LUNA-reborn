...
*/
	uint16_t GetFileNameSize()const {return (uint16_t)m_pszFileName.GetSize();}

/**
	\return	the comment size in characters (without NULL)
*/
	uint16_t GetCommentSize() const {return (uint16_t)m_pszComment.GetSize();}

/**
	\return	the extra field size in characters
*/
	uint16_t GetExtraFieldSize()const{return (uint16_t)m_pExtraField.GetSize();}
...
*/
	bool IsDataDescr()const {	return (m_uFlag & (uint16_t) 8) != 0;}
	...
	*/
	...
	{
		uint32_t uBefore = m_uUncomprSize;
		// ignore the length of encryption header 
		uint32_t uAfter = GetEffComprSize();
...
*/
	bool IsEncrypted()const {	return (m_uFlag & (uint16_t) 1) != 0;}

	char m_szSignature[4];			///< central file header signature
	uint16_t m_uVersionMadeBy;			///< version made by and system compatibility
	uint16_t m_uVersionNeeded;			///< version needed to extract
	uint16_t m_uFlag;					///< general purpose bit flag  
	uint16_t m_uMethod;					///< compression method 
	uint16_t m_uModTime;				///< last mod file time
	uint16_t m_uModDate;				///< last mod file date 
	uint32_t m_uCrc32;					///< crc-32    
	uint32_t m_uComprSize;				///< compressed size 
	uint32_t m_uUncomprSize;			///< uncompressed size     
//         filename length                 2 bytes
// 	uint16_t m_uFileNameSize;
//         extra field length              2 bytes
// 	uint16_t m_uExtraFieldSize;
//         file comment length             2 bytes
// 	uint16_t m_uCommentSize;
	uint16_t m_uDiskStart;				///< disk number start
	uint16_t m_uInternalAttr;			///< internal file attributes 
protected:
	uint32_t m_uExternalAttr;			///< external file attributes 
public:
	uint32_t m_uOffset;				///< relative offset of local header 
...
*/
	uint32_t GetSize(bool bLocal = false)const;
...
*/
	...
	uint32_t GetSystemAttr()const;

	/**
		\return 
		the original attributes as stored in the archive (without performing any conversion)
	*/
	uint32_t GetOriginalAttributes() const {return m_uExternalAttr;}
...
*/
	void SetSystemCompatibility(int iSystemID)
	{
		...
		m_uVersionMadeBy |= (uint16_t)(iSystemID << 8);
	}
...
*/
	void SetSystemAttr(uint32_t uAttr);

/**
	Set the version made by number.

*/
	void SetVersion(uint16_t uVersion)
	{
		if ((m_uVersionMadeBy & 0x00FF) != (uVersion & 0x00FF)) 
		{
			m_uVersionMadeBy &= 0xFF00;
			m_uVersionMadeBy |= (uint16_t)(uVersion & 0x00FF);
		}
	}
...
/**
	...
	\note Throws exceptions.
*/
	bool ReadLocal(CZipStorage *pStorage, uint16_t& iLocExtrFieldSize);
/**
	...
	\note Throws exceptions.
*/
	uint32_t Write(CZipStorage *pStorage);
