...
CZipException::CZipException(int iCause, const char* lpszZipName)
#ifdef _MFC_VER
	:CException(TRUE)
#endif
{
...
// inline void CZipException::Throw(int iZipError, const char* lpszZipName)
// {
// #ifdef _MFC_VER
...
int32_t CZipException::GetErrorMessage(char* lpszError, uint32_t nMaxError,
	uint32_t* )

{
	...
	if (sz.IsEmpty())
		return FALSE;
	uint32_t iLen = sz.GetLength();
	if (nMaxError - 1 < iLen)
		iLen = nMaxError - 1;
	char* lpsz = sz.GetBuffer(iLen);
#ifdef _UNICODE	
...
CZipString CZipException::GetSystemErrorDescription()
{
	...
	if (x)
	{
		...
		CZipString sz = (const char*)lpMsgBuf;
		LocalFree(lpMsgBuf);
		return sz;
	}
#endif
...
CZipString CZipException::GetInternalErrorDescription(int iCause, bool bNoLoop)
{
	...
	switch (iCause)
	{
		...
		case internal:
			...
			break;
		case notRemoved:
			sz.Format(_T("%s (%s)"), _T("Error while removing a file"), (const char*)GetSystemErrorDescription());
			break;
		case notRenamed:
			sz.Format(_T("%s (%s)"), _T("Error while renaming a file"), (const char*)GetSystemErrorDescription());
			break;
		case platfNotSupp:
		...
		case versionError:
			...
			break;
		default:
			sz = bNoLoop ? _T("Unknown error") :(const char*) GetSystemErrorDescription();
	}
	return sz;
}
