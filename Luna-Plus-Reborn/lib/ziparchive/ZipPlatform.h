...
*/
	ZIP_API uint32_t GetDefaultAttributes(); 

/**
	\return	the default directory attributes for the current system
*/
	ZIP_API uint32_t GetDefaultDirAttributes();

/**
	Get the free space on the device pointed by \e lpszPath
*/
	ZIP_API uint32_t GetDeviceFreeSpace(const char* lpszPath);
...
*/
	ZIP_API CZipString GetTmpFileName(const char* lpszPath = NULL, uint32_t iSizeNeeded = uint32_t(-1));
/**
...
*/
	...
	ZIP_API bool ChangeDirectory(const char* lpDirectory);
	ZIP_API bool SetFileAttr(const char* lpFileName, uint32_t uAttr);
	ZIP_API bool GetFileAttr(const char* lpFileName, uint32_t& uAttr);
	ZIP_API bool GetFileModTime(const char* lpFileName, time_t & ttime); ///< get the file modification time
	ZIP_API bool GetFileSize(const char* lpszFileName, uint32_t& dSize);
	ZIP_API bool SetFileModTime(const char* lpFileName, time_t ttime);	 ///< set the file modification time
	ZIP_API bool CreateDirectory(const char* lpDirectory);
	ZIP_API bool SetVolLabel(const char* lpszPath, const char* lpszLabel); ///< \c lpszPath may point to a file on the device
	ZIP_API bool ForceDirectory(const char* lpDirectory);	///< create nested directories at once
	ZIP_API bool RemoveFile(const char* lpszFileName, bool bThrow = true);
	ZIP_API bool RenameFile( const char* lpszOldName, const char* lpszNewName, bool bThrow = true);

#ifndef _MFC_VER
	ZIP_API bool TruncateFile(int iDes, uint32_t iSize);
	ZIP_API int OpenFile(const char* lpszFileName, uint32_t iMode, int iShareMode);
...
*/
	ZIP_API bool DirectoryExists(const char* lpszDir);
/**
	\param	lpszFilePath
...
*/
	ZIP_API bool IsDriveRemovable(const char* lpszFilePath);
/**
	\param	uAttr
		attributes value to test		
	\return	\c true if the attributes are the directory attributes
*/
	ZIP_API bool IsDirectory(uint32_t uAttr);
...
*/
	ZIP_API int FileExists(const char* lpszName);
...
*/
	ZIP_API int WideToSingle(const char* lpWide, CZipAutoBuffer &szSingle);
/**
