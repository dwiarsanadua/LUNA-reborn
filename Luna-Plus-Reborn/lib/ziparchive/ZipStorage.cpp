...
uint32_t CZipStorage::Read(void *pBuf, uint32_t iSize, bool bAtOnce)
{
	if (iSize == 0)
		return 0;
	uint32_t iRead = 0;
	...
	{
		ChangeDisk(m_iCurrentDisk + 1);
		uint32_t iNewRead = m_pFile->Read((char*)pBuf + iRead, iSize - iRead);
...
void CZipStorage::Open(const char* szPathName, int iMode, int iVolumeSize)
{
...
bool CZipStorage::OpenFile(const char* lpszName, uint32_t uFlags, bool bThrow)
{
...
CZipString CZipStorage::GetTdVolumeName(bool bLast, const char* lpszZipName) const
{
	CZipString szFilePath = lpszZipName ? lpszZipName : (const char*)m_pFile->GetFilePath();
...
void CZipStorage::NextDisk(int iNeeded, const char* lpszFileName)
{
	Flush();
	ASSERT(m_iSpanMode != noSpan);
	if (m_iBytesWritten)
	...
	if (bPkSpan)
		szFileName  = lpszFileName ? lpszFileName : (const char*)m_pFile->GetFilePath();
	else
...
uint32_t CZipStorage::GetFreeVolumeSpace() const
{
...
void CZipStorage::UpdateSpanMode(uint16_t uLastDisk)
{
...
void CZipStorage::Write(const void *pBuf, uint32_t iSize, bool bAtOnce)
{
	if (!IsSpanMode())
	...
	else
	{
		...
		uint32_t iNeeded = bAtOnce ? iSize : 1; 
		uint32_t uTotal = 0;
		...
		while (uTotal < iSize)
		{
			...
			uint32_t uLeftToWrite = iSize - uTotal;
			uint32_t uToWrite = uFree < uLeftToWrite ? uFree : uLeftToWrite;
...
void CZipStorage::WriteInternalBuffer(const char *pBuf, uint32_t uSize)
{
	uint32_t uWritten = 0;
	while (uWritten < uSize)
	{
		uint32_t uFreeInBuffer = GetFreeInBuffer();
		if (uFreeInBuffer == 0)
		{
			Flush();
			uFreeInBuffer = m_pWriteBuffer.GetSize();
		}
		uint32_t uLeftToWrite = uSize - uWritten;
		uint32_t uToCopy = uLeftToWrite < uFreeInBuffer ? uLeftToWrite : uFreeInBuffer;
...
uint32_t CZipStorage::VolumeLeft() const
{
