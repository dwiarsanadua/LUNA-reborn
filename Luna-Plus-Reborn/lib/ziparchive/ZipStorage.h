...
{
	uint32_t m_uDiskNeeded;		///< the number of a disk needed (counting from 1)
};
...
struct ZIP_API CZipActionCallback : public CZipCallback
{
	...
	*/
	virtual void Init(const char* lpszFileInZip = NULL, const char* lpszExternalFile = NULL) 
	{
	...
	*/
	virtual void SetTotal(uint32_t uTotalToDo)
	{
...
// 		m_uTotalSoFar = 0; // already done in CZipArchive::CZipClbckStrg::Get
	...
	*/
	uint32_t m_uTotalToDo;				
	uint32_t m_uTotalSoFar;			///< total number of bytes processed so far 
	...
	*/
	uint32_t LeftToDo() {return m_uTotalToDo - m_uTotalSoFar;}
...
/**
	...
	\see CZipArchive::Open(const char*, int, int)
*/
...
*/
	void Open(const char* szPathName, int iMode, int iVolumeSize);
...
*/
	void UpdateSpanMode(uint16_t uLastDisk);
...
/**
	...
	\note Throws exceptions.
*/
	void Write(const void *pBuf, uint32_t iSize, bool bAtOnce);
...
/**
	...
	\note Throws exceptions.
*/
	uint32_t Read(void* pBuf, uint32_t iSize, bool bAtOnce);

	/**
		Return the position in the file, taking into account the bytes in the write buffer.
		\note Throws exceptions.
	*/
	uint32_t GetPosition() const {return (uint32_t)(m_pFile->GetPosition()) + m_uBytesInWriteBuffer;}
...
/**
	...
	\note Throws exceptions.
*/
	void NextDisk(int iNeeded, const char* lpszFileName = NULL);
...
*/
	bool OpenFile(const char* lpszName, uint32_t uFlags, bool bThrow = true);
/**
...
/**
	...
	\note Throws exceptions.
*/
	void WriteInternalBuffer(const char *pBuf, uint32_t uSize);

/**
	\return	the number of free bytes on the current removable disk
*/
	uint32_t GetFreeVolumeSpace() const;
...
/**
	...
	\return	
		the new volume name
*/
	CZipString GetTdVolumeName(bool bLast, const char* lpszZipName = NULL) const;
	...
	*/
	uint32_t GetFreeInBuffer() const {return m_pWriteBuffer.GetSize() - m_uBytesInWriteBuffer;}
	
	/**
		Number of bytes available in the write buffer.		
	*/
	uint32_t m_uBytesInWriteBuffer;
...
*/
	uint32_t m_uCurrentVolSize;


	/**
		number of bytes left free in the write buffer		
	*/
	uint32_t m_uVolumeFreeInBuffer;
