...
// 	m_pszFileName.Release();
	uint16_t uFileNameSize, uCommentSize, uExtraFieldSize;
...
uint32_t CZipFileHeader::Write(CZipStorage *pStorage)
{
	uint16_t uFileNameSize = GetFileNameSize(), uCommentSize = GetCommentSize(),
		uExtraFieldSize = GetExtraFieldSize();
	uint32_t iSize = FILEHEADERSIZE + uFileNameSize + uCommentSize + uExtraFieldSize;
...
bool CZipFileHeader::ReadLocal(CZipStorage *pStorage, uint16_t& iLocExtrFieldSize)
{
	...
	bool bIsDataDescr = (((uint16_t)*(buf + 6)) & 8) != 0;
	
	uint16_t uFileNameSize = GetFileNameSize();
	uint16_t uTemp; 
...
void CZipFileHeader::SetTime(const time_t & ttime)
{
    ...
    uint16_t year = (uint16_t)(gt->tm_year + 1900);
    if (year <= 1980)
		year = 0;
	else
		year -= 1980;
    m_uModDate = (uint16_t) (gt->tm_mday + ((gt->tm_mon + 1)<< 5) + (year << 9));
    m_uModTime = (uint16_t) ((gt->tm_sec >> 1) + (gt->tm_min << 5) + 
		(gt->tm_hour << 11));
}
...
void CZipFileHeader::WriteLocal(CZipStorage& storage)
{
	...
	uint16_t uFileNameSize = GetFileNameSize(),	uExtraFieldSize = GetExtraFieldSize();
	uint32_t iLocalSize = LOCALFILEHEADERSIZE + uExtraFieldSize + uFileNameSize;
	...
	m_uDiskStart = (uint16_t)storage.GetCurrentDisk();
...
uint32_t CZipFileHeader::GetSize(bool bLocal)const
{
	if (bLocal)
...
bool CZipFileHeader::SetComment(const char* lpszComment)
{
...
bool CZipFileHeader::SetFileName(const char* lpszFileName)
{
...
uint32_t CZipFileHeader::GetSystemAttr()const
{
...
void CZipFileHeader::SetSystemAttr(uint32_t uAttr)
{
