...
{
	if (m_nBufSize < (uint32_t)nGrowTo)
	{
		if (m_nGrowBy == 0)
		...
		uint8_t* lpNew;
		if (m_lpBuf)
			lpNew = (uint8_t*)realloc((void*) m_lpBuf, nNewSize);
		else
			lpNew = (uint8_t*)malloc(nNewSize);
...
{
	if (m_nBufSize < (uint32_t)nNewLen)
		Grow((size_t)nNewLen);
	else
...
uint32_t CZipMemFile::Read(void *lpBuf, uint32_t nCount)
{
	if (m_nPos > m_nDataSize)
		return 0;
	uint32_t nToRead = (m_nPos + nCount > m_nDataSize) ? m_nDataSize - m_nPos : nCount;
...
void CZipMemFile::Write(const void *lpBuf, uint32_t nCount)
{
	if (!nCount)
		return;
