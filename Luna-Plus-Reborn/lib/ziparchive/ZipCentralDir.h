...
class ZIP_API CZipCentralDir  
{
	...
	*/
	struct ZIP_API CZipFindFast
	{
		...
		CZipFindFast(CZipFileHeader* pHeader, uint16_t uIndex):m_pHeader(pHeader), m_uIndex(uIndex){}
		/**
...
public:
	...
	*/
	struct ZIP_API Info
	{
		uint32_t m_uCentrDirPos;	///< the position of the beginning of the central directory (as located by #Locate)
		uint32_t m_uBytesBeforeZip;///< The number of bytes before the actual zip archive in a file. It is non-zero for self-extracting archives.
		uint16_t m_uThisDisk;		///< number of the disk with the central directory end record (the number of disks in the disk-spanning archive)
		uint16_t m_uDiskWithCD;		///< number of the disk with the start of the central directory
		uint16_t m_uDiskEntriesNo;	///< total number of entries in the central dir on this disk
		uint16_t m_uEntriesNumber;	///< total number of entries in the central dir
		uint32_t m_uSize;			///< size of the central directory (valid only if #m_bOnDisk is \c true; use #GetSize instead)
		uint32_t m_uOffset;		///< offset of start of central directory with respect to the starting disk number 
	...
	protected:
		...
		bool CheckIfOK_1()
		{
			return ((uint32_t)m_uCentrDirPos >= m_uOffset + m_uSize);
		}
		...
		{
			m_uThisDisk = (uint16_t)iCurrentDisk;
			if (m_uEntriesNumber)
			{
				m_uDiskEntriesNo = 0;	
			}
...
*/
	...
	void RemoveLastFile(CZipFileHeader* pHeader = NULL, int iIndex = -1)
	{
		...
		uint32_t uNewSize = pHeader->m_uOffset + GetBytesBefore();
		// then remove
...
*/
	uint32_t GetSize(bool bWhole = false) const;
	...
	*/
	int FindFile(const char* lpszFileName, bool bCaseSensitive, bool bSporadically, bool bFileNameOnly);
...
/**
	...
	\see CZipArchive::FindFile
*/
	int FindFileNameIndex(const char* lpszFileName) const;

	uint32_t GetBytesBefore() const {return m_info.m_uBytesBeforeZip;}
	/**
	...
	*/
	void RenameFile(uint16_t uIndex, const char* lpszNewName);
protected:
	/**
...
*/
	int CompareElement(const char* lpszFileName, uint16_t uIndex) const
	{
...
*/
	void InsertFindFastElement(CZipFileHeader* pHeader, uint16_t uIndex);
	...
	<B><CODE> typedef int (CZipString::*ZIPSTRINGCOMPARE)( const char* ) const; </CODE></B>
	*/
