...
const char CZipArchive::m_gszCopyright[] = {_T("ZipArchive library Copyright 2000 - 2002 Tadeusz Dracz")};
...
void CZipArchive::Open(const char* szPathName, int iMode, int iVolumeSize)
{
	if (!IsClosed())
	{
...
void CZipArchive::ThrowError(int err, bool bZlib)
{
	...
	CZipException::Throw(err, IsClosed() ? _T("") : (const char*)m_storage.m_pFile->GetFilePath());
}



bool CZipArchive::GetFileInfo(CZipFileHeader & fhInfo, uint16_t uIndex) const
{
	if (IsClosed())
	{
...
int CZipArchive::FindFile(const char* lpszFileName, int iCaseSensitive, bool bFileNameOnly)
{
	if (IsClosed())
	{
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"),__FILE__,__LINE__);
...
bool CZipArchive::OpenFile(uint16_t uIndex)
{	
	if (!m_centralDir.IsValidIndex(uIndex))
	{
		ASSERT(FALSE);
	...
	uint16_t uMethod = CurrentFile()->m_uMethod;
...
void* CZipArchive::_zliballoc(void* opaque, uint32_t items, uint32_t size)
{
...
uint32_t CZipArchive::ReadFile(void *pBuf,     
                            uint32_t iSize)
{
	if (m_iFileOpened != extract)
	{
	...
	while (m_info.m_stream.avail_out > 0 || (bForce && m_info.m_uComprLeft > 0))
	{
		...
		{
			uint32_t uToRead = m_info.m_pBuffer.GetSize();
		...
		{
			uint32_t uToCopy = m_info.m_stream.avail_out < m_info.m_stream.avail_in 
		...
		{
			uint32_t uTotal = m_info.m_stream.total_out;
			Bytef* pOldBuf =  m_info.m_stream.next_out;
			int err = inflate(&m_info.m_stream, Z_SYNC_FLUSH);
			uint32_t uToCopy = m_info.m_stream.total_out - uTotal;
...
int CZipArchive::CloseFile(const char* lpszFilePath, bool bAfterException)
{
	if (m_iFileOpened != extract)
	{
...
bool CZipArchive::OpenNewFile(CZipFileHeader & header,     
                              int iLevel,                  
                              const char* lpszFilePath, uint32_t uInternal)
{
	if (IsClosed())
	{
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"),__FILE__,__LINE__);
	...
	if (GetCount() ==(uint16_t)USHRT_MAX)
	{
		TRACE(_T("%s(%i) : Maximum file count inside archive reached.\n"),__FILE__,__LINE__);
		return false;
	}
	
	uint32_t uAttr = 0; // ..compiler
	time_t ttime;
	if (lpszFilePath)
	{
...
bool CZipArchive::ExtractFile(uint16_t uIndex,                  
                              const char* lpszPath,             
                              bool bFullPath,              
                              ...
                              uint32_t nBufSize)
{
	...
	CZipString szFileNameInZip = (const char*)header.GetFileName();
...
void CZipArchive::SetExtraField(const char *pBuf, uint16_t iSize)
{
	if (m_iFileOpened != compress)
	{
...
bool CZipArchive::WriteNewFile(const void *pBuf, uint32_t iSize)
{
	if (m_iFileOpened != compress)
	{
		TRACE(_T("%s(%i) : A new file must be opened.\n"),__FILE__,__LINE__);
    ...
    while (m_info.m_stream.avail_in > 0)
    {
        ...
        {
            uint32_t uToCopy = (m_info.m_stream.avail_in < m_info.m_stream.avail_out) 
...
void CZipArchive::DeleteFile(uint16_t uIndex)
{
...
void CZipArchive::DeleteFiles(CZipWordArray &aIndexes)
{
	...
	uint32_t uTotalToMoveBytes = 0, uLastOffset = m_storage.m_pFile->GetLength() - m_centralDir.GetBytesBefore();
	...
	uint32_t uMoveBy = 0, uOffsetStart = 0;
	for (i = 0; i < uSize; i++)
	{
		const CZipDeleteInfo& di = aInfo[i];
		...
		{
			// next hole
			uint32_t uTemp = di.m_pHeader->m_uOffset;
...
bool CZipArchive::AddNewFile(const char* lpszFilePath,
                             int iComprLevel,          
                             bool bFullPath,
							 int iSmartLevel,
...
bool CZipArchive::AddNewFile(CZipAddNewFileInfo& info)
{
	...
	if (bNeedTempArchive && (bEffInMem || 
		!(szTempFileName = ZipPlatform::GetTmpFileName(
			m_szTempPath.IsEmpty() ? NULL : (const char*)m_szTempPath, iFileSize)
		).IsEmpty()))
	{
...
CZipString CZipArchive::GetGlobalComment() const
{
	...
	CZipString temp;	
	return SingleToWide(m_centralDir.m_pszComment, temp) != -1 ? (const char*)temp : _T("");
}

bool CZipArchive::SetGlobalComment(const char* lpszComment)
{
	if (IsClosed())
	{
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"),__FILE__,__LINE__);
...
bool CZipArchive::SetFileComment(uint16_t uIndex, const char* lpszComment)
{
	if (IsClosed())
	{
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"),__FILE__,__LINE__);
...
void CZipArchive::CryptInitKeys()
{
	...
	m_keys[1] = 591751049L;
	m_keys[2] = 878082192L;
	for (uint32_t i = 0; i < m_pszPassword.GetSize(); i++)
...
bool CZipArchive::CryptCheck()
{
	...
	return CurrentFile()->IsDataDescr() ?
		(uint8_t(CurrentFile()->m_uModTime >> 8) == b) : (uint8_t(CurrentFile()->m_uCrc32 >> 24) == b);
}
...
bool CZipArchive::SetPassword(const char* lpszPassword)
{
...
uint32_t CZipArchive::CryptCRC32(uint32_t l, char c)
{
	const uint32_t *CRC_TABLE = get_crc_table();
...
{
	if (CurrentFile()->IsEncrypted())
		for (uint32_t i = 0; i < m_info.m_uComprLeft; i++)
...
bool CZipArchive::TestFile(uint16_t uIndex, uint32_t uBufSize)
{
	if (m_storage.IsSpanMode() == 1)
	{
	...
	if (pHeader->IsDirectory())
	{
		...
		uint32_t iSize = pHeader->m_uComprSize;
...
int CZipArchive::WideToSingle(const char* lpWide, CZipAutoBuffer &szSingle)
{
#ifdef _UNICODE
...
void CZipArchive::CryptDecodeBuffer(uint32_t uCount)
{
	if (CurrentFile()->IsEncrypted())
...
void CZipArchive::SetFileHeaderAttr(CZipFileHeader& header, uint32_t uAttr)
{
	header.SetSystemCompatibility(m_iArchiveSystCompatib);
...
void CZipArchive::SetRootPath(const char* szPath)
{
	if (IsClosed())
	{
		TRACE(_T("%s(%i) : Set it after opening the archive.\n"),__FILE__,__LINE__);
...
bool CZipArchive::RemovePathBeginning(const char* lpszBeginning, CZipString& szPath, ZIPSTRINGCOMPARE pCompareFunction)
{
...
void CZipArchive::SetTempPath(const char* lpszPath, bool bForce)
{
...
CZipString CZipArchive::PredictFileNameInZip(const char* lpszFilePath, 
...
CZipString CZipArchive::PredictExtractedFileName(const char* lpszFileNameInZip, const char* lpszPath, bool bFullPath, const char* lpszNewName)const 
{
...
bool CZipArchive::CWildcard::IsPatternValid(const char* lpszPattern, int* iErrorType)
{
	try
	{
...
bool CZipArchive::CWildcard::IsPattern(const char* lpszPattern)
{
	while (*lpszPattern)
	{
        switch (*lpszPattern++)
...
bool CZipArchive::CWildcard::IsMatch(const char* lpszText, int *iRetCode)
{
	CZipString sz;
	if (!m_bCaseSensitive)
	{
		sz = lpszText;
		sz.MakeLower();
		lpszText = (const char*)sz;
	}
	int i = Match((const char*)m_szPattern, lpszText);
	if (iRetCode)
		*iRetCode = i;
	return i == matchValid;
}

int CZipArchive::CWildcard::MatchAfterStar(const char* p, const char* t)
{
...
int CZipArchive::CWildcard::Match(const char* lpszPattern, const char* lpszText)
{
...
void CZipArchive::FindMatches(const char* lpszPattern, CZipWordArray &ar, bool bFullPath) const
{
	if (IsClosed())
	{
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"),__FILE__,__LINE__);
...
int CZipArchive::WillBeDuplicated(const char* lpszFilePath, bool bFullPath, bool bFileNameOnly , int iWhat)
{
	CZipString szFile;
...
bool CZipArchive::GetFromArchive(CZipArchive& zip, uint16_t uIndex, int iReplaceIndex, bool bKeepSystComp, CZipActionCallback* pCallback)
{
	...
	uint32_t uStartOffset = zip.m_centralDir.GetBytesBefore() + fh.m_uOffset + fh.GetSize(true);
	uint32_t uTotalToMove = uEndOffset - uStartOffset, uTotalMoved = 0;
		
	uint32_t uPredictedSize = fh.m_uComprSize + 
	...
	if (bConvertSystem)
	{
		uint32_t uAttr = fh.GetSystemAttr();
		fh.SetSystemCompatibility(m_iArchiveSystCompatib);
	...
	uint32_t uPack = uTotalToMove > m_info.m_pBuffer.GetSize() ? m_info.m_pBuffer.GetSize() : uTotalToMove;
...
bool CZipArchive::RenameFile(uint16_t uIndex, const char* lpszNewName)
{
	if (IsClosed())
	{
		TRACE(_T("%s(%i) : ZipArchive is closed.\n"),__FILE__,__LINE__);
	...
	uint16_t uFileNameLen = fh.GetFileNameSize();
	uint16_t uNewFileNameLen = fhNew.GetFileNameSize();
	...
	if (iDelta != 0)
	{
		...
		m_info.Init();
		uint32_t uStartOffset = fh.m_uOffset + 30 + uFileNameLen;
		uint32_t uFileLen = m_storage.m_pFile->GetLength();
		uint32_t uEndOffset = uFileLen - m_centralDir.GetBytesBefore();
		...
		uint16_t uExtraFieldSize = fh.GetExtraFieldSize();
...
bool CZipArchive::UpdateReplaceIndex(int& iReplaceIndex, const char* lpszNewFileName)
{
	if (iReplaceIndex == -2)
...
void CZipArchive::MakeSpaceForReplace(int iReplaceIndex, uint32_t uTotal, const char* lpszFileName)
{
		
	ASSERT(iReplaceIndex < GetCount() - 1);
	uint32_t uReplaceStart = m_storage.m_pFile->GetPosition() - m_centralDir.GetBytesBefore();
	uint32_t uReplaceEnd = m_centralDir.m_headers[iReplaceIndex + 1]->m_uOffset;
	uint32_t uReplaceTotal = uReplaceEnd - uReplaceStart;
	...
	if (iDelta != 0)
	{
		...
		uint32_t uFileLen = m_storage.m_pFile->GetLength();
		uint32_t uUpperLimit = uFileLen - m_centralDir.GetBytesBefore(); // will be added in MovePackedFiles
		if (pCallback)
		{
...
void CZipArchive::MovePackedFiles(uint32_t uStartOffset, uint32_t uEndOffset, uint32_t uMoveBy, CZipActionCallback* pCallback, bool bForward)
{
	ASSERT(m_info.m_pBuffer.GetSize() > 0);
	...
	uint32_t uTotalToMove = uEndOffset - uStartOffset;
	uint32_t uPack = uTotalToMove > m_info.m_pBuffer.GetSize() ? m_info.m_pBuffer.GetSize() : uTotalToMove;
	char* buf = (char*)m_info.m_pBuffer;
	...
	do
	{
		...
		uint32_t uPosition = bForward ? uEndOffset - uPack : uStartOffset;
